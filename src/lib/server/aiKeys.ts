...
export const PROVIDERS: Record<AiProvider, { label: string; pattern: RegExp; modelsUrl: string; consoleUrl: string }> = {
  groq: { label: 'Groq', pattern: /^gsk_[A-Za-z0-9_-]{16,}$/, modelsUrl: 'https://api.groq.com/openai/v1/models', consoleUrl: 'https://console.groq.com/keys' },
  openai: { label: 'OpenAI', pattern: /^sk-[A-Za-z0-9_-]{16,}$/, modelsUrl: 'https://api.openai.com/v1/models', consoleUrl: 'https://platform.openai.com/api-keys' },
}
...
export async function validateProviderKey(provider: AiProvider, key: string): Promise<{ ok: true; verified: boolean } | { ok: false; reason: string }> {
  ...
  try {
    const res = await fetch(meta.modelsUrl, { headers: { Authorization: `Bearer ${key}` }, signal: AbortSignal.timeout(8000) })
